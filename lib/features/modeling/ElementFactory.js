...
ElementFactory.prototype.create = function(elementType, attrs) {
  ...
  if (/^custom\:/.test(type)) {
    ...
    return this.baseCreate(elementType,
      assign({ type: type, businessObject: type }, attrs, size));
  }
...
ElementFactory.prototype._getCustomElementSize = function (type) {
  var shape = type.replace(/^custom\:/, '');

  var sizes = {
    triangle: { width: 40, height: 40 }
  };

  return sizes[shape] || { width: 80, height: 80 };
}
