...
function BpmnRenderer(eventBus, styles, pathMap, defaultRenderer) {
  ...
  this.getRoundRectPath = function(shape) {
  ...
  this.getDiamondPath = function(shape) {
  ...
  BpmnRenderer.prototype.getShapePath = function(element) {
    var obj = this.getSemantic(element);

    if (obj.$instanceOf('bpmn:Event')) {
      return this.getCirclePath(element);
    }

    if (obj.$instanceOf('bpmn:Activity')) {
      return this.getRoundRectPath(element);
    }

    if (obj.$instanceOf('bpmn:Gateway')) {
      return this.getDiamondPath(element);
    }

    return this.getRectPath(element);
  };
...
BpmnRenderer.$inject = [ 'eventBus', 'styles', 'pathMap', 'defaultRenderer' ];
